...
_compound_factor{F <: Frequency}(::CompoundedCompounding, rate::Float64, time_frac::Float64, freq::F) = (1.0 + rate / QuantJulia.Time.value(freq)) ^ (QuantJulia.Time.value(freq) * time_frac)
...
_implied_rate{F <: Frequency}(::CompoundedCompounding, compound::Float64, time_frac::Float64, freq::F) = (compound ^ (1.0 / QuantJulia.Time.value(freq) * time_frac) - 1.0) * QuantJulia.Time.value(freq)
